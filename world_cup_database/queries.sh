...
echo  "$($PSQL "WITH one AS (SELECT name FROM games FULL JOIN teams ON games.opponent_id=teams.team_id WHERE year=2014 and round='Eighth-Final' UNION SELECT name FROM games FULL JOIN teams ON games.winner_id=teams.team_id WHERE year=2014 and round='Eighth-Final')SELECT * FROM one ORDER BY name")"
